/**
 * @file include/kernel/ar.h
 * 
 * @ingroup ar
 */

#ifndef _KERNEL_AR_H_
#define _KERNEL_AR_H_

#include <kernel/types.h>

#define AR_MAG   "!<arch>\n"		/*!< AR MAGIC ID */
#define AR_FMAG  "`\n"			/*!< AR HDR END MAGIC */

#define AR_STR_TERM		'/'	/*!< AR name string terminator */
#define GNU_AR_STR_TBL_MAG	"//"	/*!< marks a gnu archive string table */
#define GNU_AR_SYM_TBL_MAG	"/ "	/*!< marks a gnu archive symbol table */

/**
 * @brief an AR header
 */
struct ar_hdr {
	char ar_name[16];	/*!< ar file identifier */
	char ar_date[12];	/*!< file date, dec seconds since epoch */
	char ar_uid[6];		/*!< user ID; dec */
	char ar_gid[6];		/*!< group ID; dec */
	char ar_mode[8];	/*!< file mode; oct */
	char ar_size[10];	/*!< file size; dec */
	char ar_fmag[2];	/*!< AR_FMAG (0x60, 0x0a) */
};


/**
 * @brief the archive reference structure
 */
struct archive {
	char  *ar_base;		/*!< the archive's memory location */
	unsigned long ar_size;	/*!< the size of the archive in bytes */
	char  *ar_str;		/*!< the archive's string table */

	unsigned int n_file;	/*!< the number of files in the archive */
	char **fname;		/*!< pointer array to the file name strings */
	char **p_file;		/*!< pointer array to the file(s) */
	unsigned long *filesz;	/*!< the size of each file */
	unsigned long *fnamesz;	/*!< the size of each file name */

	uint32_t n_sym;		/*!< the number of symbols in the index */
	char **sym;		/*!< pointer array to the symbol name strings */
	void **p_sym;		/*!< pointer array to the file a symbol is in */

};


/**
 * @brief print all files in an archive
 * @param a: the archive to print
 */
void ar_print_files(struct archive *a);

/**
 * @brief print all symbols in an archive's symbol table
 * @param a: the archive to print symbols from
 */
void ar_print_symbols(struct archive *a);

/**
 * @brief get a newline-separated list of file names in the archive
 * @param a: the archive to query
 * @return pointer to the file name list string
 */
char *ar_get_file_list(struct archive *a);

/**
 * @brief find a file by name in the archive
 * @param a: the archive to search
 * @param name: file name to find
 * @return pointer to the file data, or NULL if not found
 */
void *ar_find_file(struct archive *a, const char *name);

/**
 * @brief find a symbol and return the file it belongs to
 * @param a: the archive to search
 * @param name: symbol name to find
 * @return pointer to the file containing the symbol, or NULL
 */
void *ar_find_symbol(struct archive *a, const char *name);

/**
 * @brief free all resources associated with an archive
 * @param a: the archive to free
 */
void ar_free(struct archive *a);

/**
 * @brief load and parse an AR archive from memory
 * @param p: pointer to the archive data
 * @param size: size of the archive data in bytes
 * @param a: archive structure to populate
 * @return 0 on success, negative error code on failure
 */
int ar_load(char *p, unsigned long size, struct archive *a);

/**
 * @brief read a file from the archive into a buffer
 * @param a: the archive to read from
 * @param name: file name to read
 * @param ptr: destination buffer
 * @return number of bytes read, or negative error code on failure
 */
unsigned int ar_read_file(struct archive *a, const char *name, void *ptr);



#endif /* _KERNEL_AR_H_ */

