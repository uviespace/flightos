...
\externaldocument[REQ-]{FLIGHTOS-UVIE-SRS-001_Issue_1_1}
...
\uvietitlepage%
{FlightOS --\\\mbox{An RTOS for Space-Based Astronomy}}%
{\doctitle}%
...
\begin{versionhistory}
  ...
  \vhEntry{1.1}{31.08.2026}{AL}{adopt FlightOS name, add refernces to LEON3/4 targets, update design references}
\end{versionhistory}
...
This document specifies the software architecture for the real-time operating system
FlightOS. This document targets developers, testers and advanced users of
...
space. Here, the tasks of the operating system are executed.
...
FlightOS is designed with the \gls{LEON3-FT} as a target processor platform.
To allow re-use and adaptation to new hardware, the software components of
FlightOS are designed to be modular. Unless neeeded for particular drivers, all
hardware access is abstracted into a separate layer as much as possible, so
improved portability is ensured.
...
point with regard to the features of the target platform. It is designed
to be flexible enough that the operating system may be used with various
\gls{LEON3} based platforms.
...
In FlightOS, the underlying hardware is accessed in multiple layers of
...
interface. Other functional hardware components have their own driver modules.
These are in turn used by the higher level modules in the operating system and
...
{\meetsreq{GEN-0006, GEN-0009, GEN-0018, GEN-0010, FUN-0023, GEN-0402, GEN-0403}}%
...
system's exectuion. The operating system maintains these timers to
measure time or schedule kernel wakeups.

In addition to the \gls{CPU} bound timers of the \gls{LEON3-FT}, the real-time clock of the \gls{SSDP}
is supported.
}%
...
Task stacks support painting and alignment checks for validation purposes.
}%
{\meetsreq{FUN-0012, FUN-0013, FUN-0019, GEN-0401}}{}
...
an \gls{ISR} may also preempt the operating system if its release
...
The \emph{GRSPW2} \gls{SpaceWire} devices of the supported platforms are
accessible from user space via a system call interface which facilitates control
and packet exchange. 
If a \gls{SpaceWire} device is only used in the \gls{Xentium}
processing network on the \gls{SSDP}, it may be configured by the driver for direct \gls{DMA} to
the input buffer stage. Similarly, it may be used with \gls{DMA} for the output
buffer stage.

If the \gls{RMAP} feature is to be used, the configured physical memory block
is mapped into a virtual memory space.
}%
{\meetsreq{FUN-0025, FUN-0026, FUN-0027, FUN-0028}}{}


\design{GEN-0013}%
{\gls{SSDP} \gls{ADC}/\gls{DAC} Driver}{%
...
\design{GEN-0018}%
{Application Loader}{%
Applications can be loaded from the operating system and also through
the the system call interface. The application supplied in\gls{ELF} binary format
matching to the processor architecture. The C-library supplied with the OS
also provides a crt0 runtime which defines a symbol that is used by the loader
to idenitify the application start instruction.

Start-up arguments can be supplied to the application during loading in the
common argc/argv format.
}%
{\meetsreq{FUN-0804}}{}


\design{GEN-0019}%
{Device Drivers and Initcalls}{%
A set of initicall helpers support self-startup of driver components during
the bootstrap phase. This is achieved via a special section in the linker
file comparable to the standard CTOR constructor calls, but selectable
startup priority.
}%
{\meetsreq{FUN-0028, FUN-0029, FUN-0030}}{}
