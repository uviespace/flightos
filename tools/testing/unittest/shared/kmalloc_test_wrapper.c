...
enum wrap_status kmalloc_test_wrapper(enum wrap_status ws)
{
...
{
	if (kmalloc_test_wrapper(QUERY) == DISABLED)
		return malloc(size);

	if (kmalloc_test_wrapper(QUERY) == SPECIAL)
