...
#define ARIEL_MRAM_BANK_SIZE		2048
#define ARIEL_MRAM_BUS_WIDTH		0	/* 8 bit width as per GR712RC-UM v2.7 p67 */
#define ARIEL_MRAM_RD_WAIT_STATES	3
#define ARIEL_MRAM_WR_WAIT_STATES	3


#define EXCHANGE_AREA_BASE_ADDR		0x6003F000	/* XXX incorrect */
...
{
	/*** XXX this is mostly disabled since we have not implemented
	 * the exchange area yet
	 */
#if 0
...
static void ariel_watchdog_handler(void *userdata)
{
	ariel_write_reset_info(0, userdata);
}

static void ariel_write_reset_info_trap(void)
{
	ariel_write_reset_info(0, NULL);
}

static int ariel_cfg_reset_traps(void)
{
	/* called by machine_halt */
	trap_handler_install(0x82, ariel_write_reset_info_trap);
	watchdog_set_handler(ariel_watchdog_handler, NULL);

	return 0;
}
late_initcall(ariel_cfg_reset_traps)
...
static int ariel_mem_cfg(void)
{
	...
	iowrite32be(0x00001260, (void *)0x80000004);
